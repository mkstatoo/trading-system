"""Paginated historical klines for backtests."""
    ...
    *,
    ...
    for _ in range(retries):
        try:
            ...
            if data.get("code") == 0:
                return sorted(data.get("data") or [], key=lambda x: x["created_at"])
        except Exception:
            time.sleep(1.0)
    return []
...
def fetch_klines_range(
    ...
    *,
    ...
    """
    CoinEx returns at most 1000 candles ending at end_time (not full range in one call).
    Walk backward until start_ms is covered.
    """
